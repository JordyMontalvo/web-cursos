...
function shouldAuthorizeCron(req) {
...
module.exports = async (req, res) => {
    ...
    // ── JOB: /api/jobs-renew-memberships (CRON) ───────────────────────────
    if (route === 'jobs-renew-memberships') {
        // CORS específico para cron (header de secreto)
        res.setHeader('Access-Control-Allow-Origin', '*');
        ...
        if (!shouldAuthorizeCron(req)) return res.status(401).json({ success: false, message: 'No autorizado' });
        ...
        for (const user of dueUsers) {
            try {
                ...
                const body = {
                    ...
                    orderId,
                    customer: { email: user.email || 'customer@example.com', reference: String(user._id) },
